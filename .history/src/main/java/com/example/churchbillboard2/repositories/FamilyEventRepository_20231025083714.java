...
public interface FamilyEventRepository extends JpaRepository<FamilyEvent, Long> {
    ...
    @Query(value = "SELECT f.id, f.family_event_date, f.family, f.family_event_type_id AS familyEventTypeId FROM family_events f WHERE (f.family_event_date BETWEEN ?1 AND ?2)", nativeQuery = true)
    ArrayList<FamilyEventWithTypeIdDTO> getFamilyEventsBetween(String startDate, String endDate);
