...
public interface FamilyEventRepository extends JpaRepository<FamilyEvent, Long> {
    ...
    @Query(value = "SELECT fe FROM FamilyEvent fe JOIN FETCH fe.familyEventType WHERE (fe.familyEventDate BETWEEN :startDate AND '2023-10-01')", 
    nativeQuery = true)
    ArrayList<FamilyEvent> getFamilyEventsBetween(String startDate, "2023-10-31");
