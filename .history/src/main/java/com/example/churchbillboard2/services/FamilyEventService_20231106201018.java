...
public class FamilyEventService {
    ...
    public InsertResult insertFamilyEvents(ArrayList<FamilyEvent> familyEvents) {
        ...
        for (FamilyEvent familyEvent : familyEvents) {
            FamilyEvent existingEvent = familyEventRepository.findByFamilyEventDateAndFamilyEventType(familyEvent.getFamilyEventDate(), familyEvent.getFamilyEventType());
            
            if (existingEvent != null) {
                // An event with the same date and type exists; update it
                ...
                // No matching event found; insert a new record
                familyEventRepository.save(familyEvent);
            }
        }

        // try {
        //     familyEventRepository.saveAll(familyEvents);
