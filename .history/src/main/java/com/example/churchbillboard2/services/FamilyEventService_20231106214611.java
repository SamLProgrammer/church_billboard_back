...
public class FamilyEventService {
    ...
    @Transactional
    public InsertResult insertFamilyEvents(ArrayList<FamilyEvent> familyEvents) {
        ...
        try {
            for (FamilyEvent familyEvent : familyEvents) {
                ...
                cal.setTime(familyEvent.getFamilyEventDate());
                ...
                if (existingEvent != null) {
                    ...
                    // existingEvent.setFamilyEventDate(familyEvent.getFamilyEventDate());
    ...
    public void fecadeInsertFamilyEvents(ArrayList<NewFamilyEventDTO> newFamilyEvents) {
    ...
    private void adjustTimeZoneToUTC(FamilyEvent familyEvent) {
        Date familyEventDate = familyEvent.getFamilyEventDate();
    
        if (familyEventDate != null) {
            // Assuming that the incoming date is in the Colombian Time Zone (COT)
            TimeZone colombianTimeZone = TimeZone.getTimeZone("America/Bogota");
            
            // Convert the date to UTC
            familyEventDate = adjustTimeZone(familyEventDate, colombianTimeZone, TimeZone.getTimeZone("UTC"));
            familyEvent.setFamilyEventDate(familyEventDate);
        }
    }
    
    private Date adjustTimeZone(Date date, TimeZone fromTimeZone, TimeZone toTimeZone) {
        long time = date.getTime();
        int offset = toTimeZone.getOffset(time) - fromTimeZone.getOffset(time);
